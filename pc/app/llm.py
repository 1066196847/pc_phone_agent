...
class LLM:
    # 其中键是字符串（str），值是 LLM 类的实例。这里使用字符串引用 "LLM" 来避免在类定义完成前引用类本身的问题
    ...
    def __init__(
        ...
        if not hasattr(self, "client"):  # Only initialize if not already initialized
            # 这里有个小bug，llm_config是LLMSettings类型，没有.get方法。但是经过查看在调用LLM的时候，没有显示初始化过，所以不会有问题
            ...
            # tiktoken是openai的开源工具，国内的模型应该是不支持的
            try:
                self.tokenizer = tiktoken.encoding_for_model(self.model)
            except KeyError:
                print("tiktoken不支持模型：", self.model)
    ...
    @retry(
        wait=wait_random_exponential(min=1, max=60), # 重试等待时间：指数退避，随机在 1 到 60 秒之间
        stop=stop_after_attempt(6), # 最多重试 6 次
        retry=retry_if_exception_type(
            (OpenAIError, Exception, ValueError)
        ),  # Don't retry TokenLimitExceeded   # 仅对 OpenAIError、Exception 和 ValueError 异常重试
    )
    async def ask(
        self,
